...
本项目通过两个核心任务验证了不同世界模型架构的有效性：

1. **MNIST 旋转预测**: 模型学习将手写数字的语义特征（数字大小）映射到空间变换（旋转角度）上，例如数字 1 旋转 10°。
2. **只狼 (Sekiro) 动态推演**: 基于 RSSM 架构在 5 万张真实游戏画面上进行的复杂高维视觉预测任务。

![MNIST VAE](doc/mnist_vae.png)
<p align="center"><b>图 1: MNIST 手写数字旋转实验结果</b></p>

![Sekiro RSSM](doc/sekiro_rssm.png)
<p align="center"><b>图 2: 只狼画面预测 (RSSM)</b></p>
<p align="center">第一行：原图；第二行：重建画面；第三行：仅以前三帧为基础的梦境推演</p>
